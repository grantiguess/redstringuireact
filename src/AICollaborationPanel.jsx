...
const AICollaborationPanel = () => {
  ...
  const initializeConnection = async () => {
    try {
      ...
      addMessage('ai', 'Claude Desktop connected! 🧠✨\n\n**Available MCP Tools (Real Redstring Store):**\n\n**📂 Graph Management:**\n• `list_available_graphs` - See all your knowledge graphs from the real Redstring store\n• `get_active_graph` - Get detailed info about the currently active graph\n• `open_graph` - Open a graph and make it active in the real Redstring UI\n• `set_active_graph` - Switch between already-open graphs\n\n**🏗️ Prototype System:**\n• `add_node_prototype` - Add a new node prototype to the global prototype pool\n• `add_node_instance` - Add a new instance of a prototype to the active graph\n\n**🎯 Real Redstring Integration:**\n• **Works with actual Redstring data** - No simulation!\n• **Persists to .redstring files** - Changes are real and permanent\n• **Updates UI immediately** - See changes in Redstring interface\n• **Uses prototype-instance system** - Proper Redstring architecture\n• **Prototype validation** - Ensures instances always have valid prototypes\n\n**I can work with your REAL Redstring knowledge graphs!** 🚀\n\n**To use these tools:**\n1. Open Claude Desktop\n2. Look for the "Search and tools" icon (slider icon)\n3. Use the Redstring MCP tools\n4. Start with `list_available_graphs` to see your real graphs!\n\nWhat would you like to explore or modify in your Redstring graphs?');
